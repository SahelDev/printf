...
int print_int (va_list ch, paramets_3 *p3, wid_per_2 *p2, flags_5 *f)
{
	...
	int l;
	/* printf("%d  %d  %d",p3->l_modif,p3->h_modif,va_arg (ch, int));*/
	if (p3->l_modif)
		l = (int)va_arg (ch, long);
	else if (p3->h_modif)
		l = (short int)va_arg (ch, int); 
	else
		l = (int)va_arg (ch, int); 
...
* Return: string
*/
char *convert (long num, int base, int flags, paramets_3 *p3, wid_per_2 *p2, flags_5 *f)
{
	static char *array;
...
int print_unsigned(va_list ap, paramets_3 *p3, wid_per_2 *p2, flags_5 *f)
{
	...
	p3->unsing = l;
	return (print_number (convert ((long int)l, 10, CONVERT_UNSIGNED, p3,p2,f) , p3,p2,f));
}
...
int print_number_right_shift (char *str,  paramets_3 *p3, wid_per_2 *p2, flags_5 *f){
	...
	if (f->flag_plus && !neg2 && pad_char == '0' && !p3->unsing)
		_putchar ('+');
	...
	if (f->flag_plus && !neg2 && pad_char == ' ' && !p3->unsing)
		n += _putchar('+');
	else if (!f->flag_plus && f->flag_space && !neg2 && !p3->unsing && !f->flag_zero) 
		n += _putchar(' ');
...
int print_number_left_shift (char *str,  paramets_3 *p3, wid_per_2 *p2, flags_5 *f){
	...
	if (f->flag_plus && !neg2 && !p3->unsing)
	{
		n += _putchar ('+');
		i++;
	
	}else if (f->flag_space && !neg2 && !p3->unsing)
	{
		n += _putchar(' ');
		i++;
	}
